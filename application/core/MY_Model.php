...
class MY_Model extends CI_Model
{
    ...
    /**
     * Get table content with all values
     * 
     * @param unknown_type $join_db_tables
     * @param unknown_type $join_catalog_tables
     * @param unknown_type $num
     * @param unknown_type $offset
     * @param unknown_type $criteria
     * @param unknown_type $order
     * @param unknown_type $where_condition
     * @return boolean
     * @autor Eduardo De Leon
     * Date 27/09/2013
     */
    public function _get_all($join_catalog_tables=array(), $join_db_tables=array(), $num = 20, $offset = 0, $criteria="", $order="DESC", $where_condition=""){
    	...
    	//Join with respective catalog tables
    	if($join_catalog_tables){
    		$index = 1;
    		foreach ($join_catalog_tables as $field=>$catalog_table){
    			$catalog 	= "catalog_".$index;
    			$table 		= "table_".$index;
    			$this->db->select($catalog.'.ca_value as '.$field."_value");
    			$this->db->join(TBL_CATALOG . ' ' . $catalog, $field . '=' . $catalog . '.' . CATALOG_ID)
    			->join(TBL_TABLE .   ' ' . $table, $catalog . '.' . CATALOG_TABLE . '=' . $table . '.' . TABLE_ID)
    			->where($table . '.' . TABLE_NAME, $catalog_table);
    			$index++;
    		}
    	}
    	
    	//Get this fields prefix
    	$primary_key 		= explode('_', $this->primary_key);
    	$fields_prefix		= element(0, $primary_key).'_'; 

    	//Join with db tables
    	if($join_db_tables){
    		foreach ($join_db_tables as $field=>$db_table){
    			//Get join table name
    			$tbl_name 	= explode("_", $db_table);
    			array_shift($tbl_name);
    			$tbl_name = implode('_',$tbl_name);
    			
    			$this->db->select($db_table.'.*');
    			$this->db->join($db_table, $field . '=' . $fields_prefix.$tbl_name);
    		}
    	}    	
    ...
    /**
     * Get one register with its respective values
     * 
     ...
     * Date 27/09/2013
     */
    public function _get($id, $join_catalog_tables=array(), $join_db_tables=array()){
    	$this->db->select($this->_table.'.*')
    	->from($this->_table)
     ...
     * Save method
     * 
     * @param unknown_type $primary_value
     * @param unknown_type $data
     * @param unknown_type $skip_validation
     * @autor Eduardo De Leon
     * Date 13/10/2013
     */
    public function save($primary_value=false, $data, $skip_validation = FALSE){
    	if($primary_value){
    		$this->update($primary_value, $data, $skip_validation = FALSE);
    	} else {
    		$this->insert($data, $skip_validation = FALSE);
    	}
    }
    ...
    function dropdown()
    {
        ...
        $options = array('none" selected="selected" disabled="disabled"'=> $this->lang->line('select'));
